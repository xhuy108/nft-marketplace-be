...
library CategoryLib {
    ...
    function removeCategory(
        ...
        for (uint i = 0; i < self.categoryNames.length; i++) {
            if (keccak256(bytes(self.categoryNames[i])) == keccak256(bytes(category))) {
                self.categoryNames[i] = self.categoryNames[self.categoryNames.length - 1];
    ...
    function getValidCategories(CategoryStorage storage self) 
        external 
        view 
